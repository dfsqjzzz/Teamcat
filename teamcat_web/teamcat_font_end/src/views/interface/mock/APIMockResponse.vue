        ...
        if (value) {
          for (let i = 0; i < this.responseList.length; i++) {
            if (this.responseList[i].Enable  && this.responseList[i].id!=responseID) {
              ...
              this.$axios.patch('/interface/interface/mock/response/' + this.responseList[i].id, {Enable: false}).then(response => {
              }, response => {
              })
            }
          }
          this.$axios.patch('/interface/interface/mock/response/' + responseID, {Enable: value}).then(response => {
          }, response => {
          })
        }
      },
